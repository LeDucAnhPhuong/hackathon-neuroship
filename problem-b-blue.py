...
class JetBotController:
    ...
    def plan_initial_route(self):
        """Lập kế hoạch đường đi ban đầu từ điểm xuất phát đến đích qua tất cả LOAD nodes."""
        rospy.loginfo(f"Đang lập kế hoạch từ node {self.navigator.start_node} đến {self.navigator.end_node} qua tất cả LOAD nodes...")
        self.planned_path = self.find_path_through_all_load_nodes(
    ...
    def setup_parameters(self):
        ...
        self.INTERSECTION_APPROACH_DURATION = 1.5
    ...
    def handle_intersection(self):
        ...
        rospy.loginfo(f"[NORMAL NODE] Node {self.current_node_id} is not LOAD type. Skipping robot turn and YOLO detection.")

        # Continue with normal intersection processing without detections
        self.process_intersection_detections([])
    ...
    def find_path_through_all_load_nodes(self, start_node_id, end_node_id, banned_edges=None):
        ...
        try:
            ...
            if not load_nodes:
                # No LOAD nodes found, use normal pathfinding
    ...
    def update_navigation_after_load_turn(self):
        """
        Update navigation state after performing East-South turn at LOAD node.
        ...
        try:
            ...
            # Continue navigation towards end node
            if self.current_node_id == self.navigator.end_node:
                rospy.loginfo("🎯 [PROBLEM B] Already at END node after LOAD handling!")
                self._set_state(RobotState.GOAL_REACHED)
            else:
                # Re-plan path from current position through remaining LOAD nodes
                self.plan_path_from_current_position()
                self._set_state(RobotState.LEAVING_INTERSECTION)

        except Exception as e:
            rospy.logerr(f"❌ Error updating navigation after LOAD turn: {e}")
            self._set_state(RobotState.DEAD_END)
    ...
    def plan_path_from_current_position(self):
        ...
        try:
            ...
            rospy.logerr(f"❌ Error planning path from current position: {e}")
            self._set_state(RobotState.DEAD_END)

    def handle_normal_intersection(self):
        """
        Handle normal intersection logic (original Problem A behavior).
        """
        current_direction = self.DIRECTIONS[self.current_direction_index]
        ...
        # Continue with the rest of the original intersection handling logic
        self.process_intersection_detections(detections)

    def process_intersection_detections(self, detections):
        """
        Process detections at intersection and make navigation decisions.
        This contains the original Problem A intersection logic.
        """
        current_direction = self.DIRECTIONS[self.current_direction_index]
        ...
        if not is_deviation:
            ...
            next_node_id = self.planned_path[self.current_path_index + 1] if self.current_path_index else self.planned_path[self.planned_path.index(self.current_node_id) + 1]
        else:
