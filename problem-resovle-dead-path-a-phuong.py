...
class JetBotController:
    ...
    def plan_initial_route(self):
        """Lập kế hoạch đường đi ban đầu từ điểm xuất phát đến đích với tránh quay 180 độ."""
    ...
    def _avoid_180_degree_turns(self, path, recursion_depth=0):
        ...
        Args:
            ...
            recursion_depth: Độ sâu đệ quy để tránh vòng lặp vô tận

        Returns:
            Đường đi mới đã tránh quay 180 độ, hoặc None nếu không tìm được
        """
    ...
    def _is_180_degree_turn(self, direction1, direction2):
        """
        Kiểm tra xem hai hướng có tạo thành quay 180 độ không.

        Args:
            direction1: Hướng đầu tiên ('N', 'E', 'S', 'W')
            direction2: Hướng thứ hai ('N', 'E', 'S', 'W')

        Returns:
            True nếu là quay 180 độ, False nếu không
        """
        opposite_directions = {
            'N': 'S', 'S': 'N',
            'E': 'W', 'W': 'E'
        ...
        return opposite_directions.get(direction1) == direction2
